...
class InMemoryMerkleTreeTest(unittest.TestCase):
    ...
    def test_in_memory_merkle_tree_incremental_sth(self):
        tree = merkle.InMemoryMerkleTree([])
        hasher = merkle.TreeHasher()
        test_vectors = InMemoryMerkleTreeTest.test_vector_data
        for i in range(len(test_vectors)):
          tree.add_leaf(test_vectors[i])
          self.assertEqual(
              tree.get_root_hash(), hasher.hash_full_tree(test_vectors[0:i+1]))

    def test_in_memory_merkle_tree_inclusion_proof_precomputed(self):
        tree = merkle.InMemoryMerkleTree(
            InMemoryMerkleTreeTest.test_vector_data)
        verifier = merkle.MerkleVerifier()
        for v in InMemoryMerkleTreeTest.precomputed_path_test_vectors:
            ...
            leaf_data = InMemoryMerkleTreeTest.test_vector_data[v.leaf]
    ...
    def test_in_memory_merkle_tree_inclusion_proof_generated(self):
        leaves = []
        leaf_hashes = []
        hasher = merkle.TreeHasher()
        for i in range(128):
        ...
        tree = merkle.InMemoryMerkleTree(leaves)
        verifier = merkle.MerkleVerifier()
